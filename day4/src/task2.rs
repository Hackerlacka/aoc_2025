...
fn run_task_on_file(path: &PathBuf) -> usize {
    ...
    let mut map = PaperRollMap::new(lines);
    map.accessible_paper_rolls_with_remove()
}

fn run_example() {
    let path = get_input_path(4, 1, InputType::Example, None);
    let accessible_paper_rolls = run_task_on_file(&path);
    assert_eq!(accessible_paper_rolls, 43);
}
...
pub fn run_task() {
    ...
    let accessible_paper_rolls = run_task_on_file(&path);
    info!("Accessible paper rolls (with remove): {accessible_paper_rolls}")
}
